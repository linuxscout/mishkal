...
#            noun_list.append(HAMZA+ALEF+noun[1:])
        for noun in noun_list:
            ...
            for seg in list_seg_comp:
                ...
                if self.debug:
                    print "\t", "-".join([procletic, 
                stem, encletic]).encode("utf8")  
        ...
        # stem reduced noun : level two
                ...
                for stem in list_stem:
                    result += self.steming_second_level(noun, stem, procletic,
                     encletic) 
    ...
    def steming_second_level(self, noun, noun2, procletic, encletic):
        ...
        for seg_conj in list_seg_conj:
            ...
            snconst.CONJ_SUFFIX_LIST_TAGS[suffix_conj]['vocalized']:
                seg_conj_voc = {'prefix':'', 'suffix':vocalized_suffix, 
                'stem':stem_conj}
        ...
        for seg_conj in list_seg_conj:
            ...
            if self.debug:
                print "*\t", "-".join([str(len(stem_conj)), \
                prefix_conj, stem_conj, suffix_conj]).encode("utf8")  
            ...
            if self.debug:
                print "\tpossible original nouns:  ", 
                "\t".join(possible_noun_list).encode('utf8') 
...
def get_suffix_variant(word, suffix, enclitic):
    ...
    if     enclitic_nm  ==   u"" and word[-1:] in (araby.ALEF_MAKSURA, 
    araby.YEH, araby.ALEF) and suffix in araby.HARAKAT  :
        suffix = u"" 
    return suffix 
